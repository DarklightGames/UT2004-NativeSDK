...
public:
	...
	void Empty( INT Slack=0 )
	{
		...
		if( GUndo )
			GUndo->SaveArray( Owner, this, 0, TArray<T>::ArrayNum, -1, sizeof(T), SerializeItem, DestructItem );
	...
	INT AddUniqueItem( const T& Item )
	{
		...
		for( INT Index=0; Index < TArray<T>::ArrayNum; Index++ )
			if( (*this)(Index)==Item )
				return Index;
		return AddItem( Item );
		unguardSlow;
	...
	INT RemoveItem( const T& Item )
	{
		...
		INT OriginalNum = TArray<T>::ArrayNum;
		for( INT Index=0; Index < TArray<T>::ArrayNum; Index++ )
			if( (*this)(Index)==Item )
				Remove( Index-- );
		return OriginalNum - TArray<T>::ArrayNum;
		unguardSlow;
	}
	...
	void ModifyAllItems()
	{
		...
		if( GUndo )
			GUndo->SaveArray( Owner, this, 0, TArray<T>::Num(), 0, sizeof(T), SerializeItem, DestructItem );
		unguardSlow;
	}
...
public:
	...
	void MultiFind( const TK& Key, TArray<TI>& Values ) 
	{
		...
		for( INT i=TMapBase<TK,TI>::Hash[(GetTypeHash(Key) & (TMapBase<TK,TI>::HashCount-1))]; i!=INDEX_NONE; i=TMapBase<TK,TI>::Pairs(i).HashNext )
			if( TMapBase<TK,TI>::Pairs(i).Key==Key )
				new(Values)TI(TMapBase<TK,TI>::Pairs(i).Value);
		unguardSlow;
	}
	TI& Add( typename TTypeInfo<TK>::ConstInitType InKey, typename TTypeInfo<TI>::ConstInitType InValue )
	{
	...
	{
		for( INT i=TMapBase<TK,TI>::Hash[(GetTypeHash(InKey) & (TMapBase<TK,TI>::HashCount-1))]; i!=INDEX_NONE; i=TMapBase<TK,TI>::Pairs(i).HashNext )
			if( TMapBase<TK,TI>::Pairs(i).Key==InKey && TMapBase<TK,TI>::Pairs(i).Value==InValue )
				return TMapBase<TK,TI>::Pairs(i).Value;
		return Add( InKey, InValue );
	}
	...
	TI* FindPair( const TK& Key, const TK& Value )
	{
		guardSlow(TMap::Find);
		for( INT i=TMapBase<TK,TI>::Hash[(GetTypeHash(Key) & (TMapBase<TK,TI>::HashCount-1))]; i!=INDEX_NONE; i=TMapBase<TK,TI>::Pairs(i).HashNext )
			if( TMapBase<TK,TI>::Pairs(i).Key==Key && TMapBase<TK,TI>::Pairs(i).Value==Value )
				return &TMapBase<TK,TI>::Pairs(i).Value;
		return NULL;
		unguardSlow;
	}
